...
namespace Gateway.Core.Providers
{
    ...
    public class VakifbankPaymentProvider : IPaymentProvider
    {
        ...
        public VakifbankPaymentProvider(IConfiguration config)
        {
            _Config = config;
        }
        ...
        public Response<TheedResult> GetPaymentParameters(AuthorizationRequest request)
        {
            ...
            string merchantId = _Config["Vakifbank:MerchantId"];
            string merchantPassword = _Config["Vakifbank:Password"];
            string successUrl = $"{_Config["YapiKredi:Host"]}/payment/callback";
            string failUrl = $"{_Config["YapiKredi:Host"]}/payment/callback";
            ...
            try
            {
                ...
                */
                ...
                httpParameters.Add("VerifyEnrollmentRequestId", request.OrderNumber);
                httpParameters.Add("SessionInfo", "1");//banka dökümanlarına göre sabit bir değer
                ...
                var enrollmentUrl = new Uri(_Config["Vakifbank:Gate"]);
                var response = new HttpClient().PostAsync(enrollmentUrl, new FormUrlEncodedContent(httpParameters)).GetAwaiter().GetResult();
                ...
                if (statusNode.InnerText != "Y")
                {
                    ...
                    return new Response<TheedResult>()
                    {
                        ...
                        ErrorMessage = $"{messageErrorNode.InnerText} - {messageErrorCodeNode?.InnerText}"
                    };
                }
        ...
        public Response<TransactionResult> GetPaymentResult(IFormCollection form)
        {
            ...
            if (StringValues.IsNullOrEmpty(status))
            {
                return new Response<TransactionResult>()
                {
                    ErrorMessage = "İşlem sonuç bilgisi alınamadı."
                };
            }
            ...
            var response = new HttpClient().PostAsync(_Config["Vakifbank:Gate"], new FormUrlEncodedContent(parameters)).GetAwaiter().GetResult();
            string responseContent = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
        ...
        Response<TransactionResult> IPaymentProvider.Sales(AuthorizationRequest request)
        {
            throw new NotImplementedException();
        }

        public TransactionResult Refund(AuthorizationRequest request)
        {
            throw new NotImplementedException();
        }

        public TransactionResult Cancel(AuthorizationRequest request)
        {
            throw new NotImplementedException();
        }
        ...
        Response<TransactionResult> IPaymentProvider.Refund(AuthorizationRequest request)
        {
            throw new NotImplementedException();
        }

        Response<TransactionResult> IPaymentProvider.Cancel(AuthorizationRequest request)
        {
            throw new NotImplementedException();
        }
